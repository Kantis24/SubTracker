    ...
    <article
      className={`rounded-2xl border p-4 transition ${
        subscription.active
          ...
          : "border-slate-800/50 bg-slate-900/25 opacity-80"
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="truncate text-base font-semibold text-white">{subscription.name}</h3>
            {!subscription.active ? (
          ...
          <p className="mt-1 text-xs text-slate-500">{listName}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-semibold tabular-nums text-white">
            {formatCurrency(subscription.amount)}
          </p>
          <p className="text-xs text-slate-500">{cycleNote}</p>
        </div>
      </div>

      <dl className="mt-3 grid gap-2 text-sm text-slate-400 sm:grid-cols-2">
        <div>
          <dt className="text-xs uppercase tracking-wide text-slate-600">Next due</dt>
          <dd className="font-medium text-slate-200">{formatShortDate(subscription.nextDueDate)}</dd>
        </div>
        <div>
          <dt className="text-xs uppercase tracking-wide text-slate-600">Est. monthly</dt>
          <dd className="font-medium tabular-nums text-slate-200">{formatCurrency(monthlyEq)}</dd>
        </div>
      </dl>

      {subscription.notes ? (
        <p className="mt-3 line-clamp-3 text-sm text-slate-500">{subscription.notes}</p>
      ) : null}
