  ...
  return (
    ...
    >
      {cards.map((c) => (
        <div
          key={c.label}
          className="rounded-2xl border border-slate-800/90 bg-slate-900/50 p-4 shadow-sm backdrop-blur-sm transition hover:border-slate-700/90"
        >
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{c.label}</p>
          <p className="mt-1 font-mono text-2xl font-semibold tabular-nums tracking-tight text-white">
            {c.value}
          </p>
          <p className="mt-3 text-xs leading-snug text-slate-500">{c.hint}</p>
        </div>
      ))}
    </section>
  );
